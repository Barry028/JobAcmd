...
var globalSetting = {
    ...
    processingImg: '<div id="svgContainer"></div>'
};
...
function lottie_loading(id) {
    var loader = lottie.loadAnimation({
        container: id,
        animType: 'svg',
        loop: true,
        autoplay: false,
        path: 'https://assets6.lottiefiles.com/packages/lf20_g4devxoa.json',
    });
    loader.play();
}

function blockUI() {
    // 2022 02 11 職務再設計要求不要有 過渡動畫
...
function unblockUI() {
    ...
    $.unblockUI();
    mask.hide();
    mask.find('#svgContainer svg').remove()
...
function blockMessage(msg, title, unblockCallback) {
    ...
    $.confirm({
        icon: 'tu-check-circle text--info',
        title: "<div class='blockMessageTitle'>" + (title ? title : '提示訊息') + "</div>",
        confirmButton: '確定',
        confirmButtonClass: 'btn--shadow--info btn--md',
        cancelButton: false,
...
function blockMessageWithCancel(msg, title, unblockCallback, cancelText, cancelCallback) {
    ...
    $.confirm({
        icon: 'tu-alert-triangle text--info',
        title: "<div class='blockMessageTitle'>" + (title ? title : '提示訊息') + "</div>",
        confirmButton: '確定',
        confirmButtonClass: 'btn--shadow--info btn--md',
        cancelButton: (cancelText == null) ? '取消' : cancelText,
        cancelButtonClass: 'btn--border--info btn--md',
        content: "<div class='blockMessage'>" + msg.replace(/(\n)+/g, '<br />') + "</div>",
...
function blockAlert(msg, title, sureCallback, height) {
    ...
    $.confirm({
        ...
        icon: 'tu-alert-triangle text--warning',
        title: "<div class='blockAlertTitle'>" + (title ? title : '提示訊息') + "</div>",
        confirmButton: '確定',
        confirmButtonClass: 'btn--shadow--warning btn--md',
        cancelButton: false,
...
function blockConfirm(msg, title, confirmCallback, cancelCallback, confirmBtnText, cancelBtnText) {
    ...
    $.confirm({
        icon: 'tu-help-circle text--danger',
        title: "<div class='blockAlertTitle'>" + (title ? title : '確認訊息') + "</div>",
        confirmButton: (confirmBtnText ? confirmBtnText : "確定"),
        confirmButtonClass: 'btn--shadow--danger btn--md',
        cancelButton: (cancelBtnText ? cancelBtnText : "取消"),
        cancelButtonClass: 'btn--border--danger btn--md',
        content: "<div class='blockAlertMessage'>" + msg.replace(/(\n)+/g, '<br />') + "</div>",
        confirm: confirmCallback,
        theme: 'white danger',
        // columnClass: "jconfirm-box-danger",
        cancel: function() {
...
//formTarget:（非必填）指定頁面要在哪個窗口顯示出來，可輸入值（"_TurboPRO"、"_self"、"_parent"、"_top"、或是其他 frame 名稱）。預設 "_self"。
...
function popupWindow(windowConfig, contentUrl, title, jsonParam, closeCallback, loadCallback, closeConfirmCallback, shownCallback) {
    try {
        ...
        var html = [
            "  <div class=\"modal fade modal-centered show\" style=\"display:block\" tabindex=\"-1\" id=\"", divId, "\" role=\"dialog\" aria-labelledby=\"popupWindow\" aria-hidden=\"true\">",
            ...
            "           <button type=\"button\" class=\"close\" style=\"margin-left:50%;margin-right:50%;\" data-dismiss=\"modal\" title=\"關閉視窗\">",
            "                <i class=\"fa fa-window-close fa-2\" aria-hidden=\"true\"></i>",
        ...
        if ($('#iframe_load').length) {
            lottie_loading(iframe_load)
        }
